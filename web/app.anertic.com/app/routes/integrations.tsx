  ...
  return [
    {
      ...
      name: 'FusionSolar',
      description:
        'Connect Huawei inverters, batteries, and smart meters via the FusionSolar Northbound API. Syncs real-time production, consumption, and battery state.',
      logo: 'FS',
      capabilities: [
        'Solar inverter',
        'Battery storage',
    ...
    },
    {
      ...
      name: 'SolarEdge',
      description:
        'Monitor SolarEdge inverters and power optimizers. Pull production data, component-level monitoring, and energy metrics.',
      logo: 'SE',
      ...
      fields: [
        {
          ...
          required: true,
          helpText:
            'Get from SolarEdge monitoring portal > Admin > Site Access',
        },
        {
          key: 'siteId',
          label: 'Site ID',
    ...
    },
    {
      ...
      name: 'Growatt',
      description:
        'Connect Growatt inverters and batteries. Supports ShineServer API for real-time monitoring and historical data.',
      logo: 'GW',
      capabilities: ['Solar inverter', 'Battery storage', 'Smart meter'],
      fields: [
        {
    ...
    },
    {
      ...
      name: 'Shelly',
      description:
        'Connect Shelly energy meters and smart plugs for real-time power monitoring and relay control.',
      logo: 'SH',
      ...
      fields: [
        ...
        },
        {
          ...
          required: true,
          helpText:
            'Get from Shelly app > User Settings > Authorization cloud key',
        },
      ],
    },
  ]
...
export default function Integrations() {
  ...
  const [connected, setConnected] = useState<Integration[]>(() =>
    generateConnectedIntegrations()
  )
  const [connectDialog, setConnectDialog] =
  ...
  const unconnected = available.filter(
    (a) => !connectedProviders.has(a.provider)
  )
  ...
  function handleSync(id: string) {
    ...
    setTimeout(() => {
      setConnected((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, status: 'connected' as IntegrationStatus } : c
        )
      )
                        ...
                        {integration.metrics.map((metric) => (
                          <div key={metric.label} className="px-4 py-3 sm:px-5">
                            <div className="flex items-center gap-1.5">
                              <metric.icon
                                className={cn('size-3', metric.color)}
                              />
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {unconnected.map((integration) => {
            ...
            return (
              <Card
                key={integration.id}
                className="group cursor-pointer py-0 transition-all hover:-translate-y-0.5 hover:shadow-md"
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This will stop syncing data from this integration. Historical data
              will be preserved. You can reconnect later.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
