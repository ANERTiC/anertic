...
export default function Devices() {
  ...
  const statusFilter =
    (searchParams.get('status') as ConnectionStatus | null) ?? 'all'
  ...
  const summary = {
    ...
    degraded: allDevices.filter((d) => d.connectionStatus === 'degraded')
      .length,
      ...
      <div className="flex items-center gap-6 rounded-xl border border-border/50 bg-muted/20 px-5 py-3.5">
        ...
        <div className="flex items-center gap-5">
          ...
          <SummaryPill
            ...
            onClick={() =>
              setFilter('status', statusFilter === 'online' ? 'all' : 'online')
            }
          />
          <SummaryPill
            ...
            onClick={() =>
              setFilter(
                'status',
                statusFilter === 'degraded' ? 'all' : 'degraded'
              )
            }
          />
          <SummaryPill
...
function DeviceRow({ device, href }: { device: DeviceListItem; href: string }) {
  ...
  return (
    ...
    >
      ...
      <RiArrowRightSLine
        aria-hidden="true"
        className="size-4 shrink-0 text-muted-foreground/30 transition-transform group-hover:translate-x-0.5 group-hover:text-muted-foreground motion-reduce:transition-none"
      />
    </Link>
  )
}
