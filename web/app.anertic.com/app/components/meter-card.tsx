...
export const PROTOCOL_CONFIG: Record<
  MeterProtocol,
  { label: string; color: string; bg: string; icon: typeof RiWifiLine }
> = {
  mqtt: {
    label: 'MQTT',
...
export const CHANNEL_CONFIG: Record<
  MeterChannel,
  { label: string; icon: typeof RiSunLine; color: string; bg: string }
> = {
  pv: {
    label: 'Solar',
    icon: RiSunLine,
  ...
  return (
    <div
      className={cn(
        ...
        expanded && 'shadow-sm ring-1 ring-border/80'
      )}
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {meter.phase > 0 && (
              <span className="rounded bg-foreground/5 px-1.5 py-0.5 font-mono text-[10px] font-bold text-foreground/70 tabular-nums">
                L{meter.phase}
              </span>
            )}
          </div>
        ...
        <div className="hidden shrink-0 items-center gap-4 sm:flex">
          {power && (
            <div className="text-right">
              <p className="font-mono text-base leading-tight font-bold tracking-tight text-foreground tabular-nums">
                {formatPower(power.value)}
              </p>
              <p className="text-[9px] font-medium tracking-widest text-muted-foreground/40 uppercase">
                power
              </p>
            </div>
          )}
          {voltage && !expanded && (
            <div className="text-right">
              <p className="font-mono text-sm leading-tight font-semibold text-foreground/70 tabular-nums">
                {formatReadingValue(voltage.value, voltage.metric)}
                <span className="ml-0.5 text-[10px] font-normal text-muted-foreground/50">
                  V
                </span>
              </p>
              <p className="text-[9px] font-medium tracking-widest text-muted-foreground/40 uppercase">
...
function TabButton({
  ...
  return (
    ...
    >
      ...
      {label}
      {active && (
        <span className="absolute right-3 bottom-0 left-3 h-[2px] rounded-full bg-foreground" />
      )}
    </button>
  )
}
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-3 gap-1.5 lg:grid-cols-6">
        {GHOST_METRICS.map((g, i) => (
          ...
          >
            <p className="text-[9px] font-semibold tracking-[0.15em] text-muted-foreground/25 uppercase">
          ...
          {isOnline ? (
            ...
            <>
              <div className="flex items-center gap-2">
                ...
                >
                  <RiPulseLine
                    className={cn('size-3', channelCfg.color, 'opacity-40')}
                  />
                </div>
              ...
              <p className="max-w-[220px] text-center text-[11px] leading-relaxed text-muted-foreground/40">
                Configure your meter to push data via {protoCfg.label} — check
                the Configure tab for connection details
              </p>
            </>
          )}
        </div>
    ...
    <>
      ...
      <div className="bg-muted/5 px-4 py-4">
        <div className="mb-2.5 flex items-center gap-1.5">
          ...
          <p className="text-[10px] font-semibold tracking-[0.15em] text-muted-foreground/50 uppercase">
            Readings
          </p>
          {meter.isOnline && (
        ...
        {readings.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-1.5 lg:grid-cols-6">
            {readings.map((reading) => {
              const isPrimary =
                reading.metric === 'power_w' || reading.metric === 'energy_kwh'
              return (
                <div
                  key={reading.metric}
                  className={cn(
                ...
                >
                  <p className="text-[9px] font-semibold tracking-[0.15em] text-muted-foreground/50 uppercase">
                    {formatMetricLabel(reading.metric)}
                  </p>
                  <p
                    className={cn(
                      'mt-1 font-mono leading-none tracking-tight tabular-nums',
                      isPrimary
                        ? 'text-base font-bold'
      ...
      <div className="border-t border-border/20 px-4 py-4">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-2 lg:grid-cols-4">
          ...
          <InfoRow
            label="Phase"
            value={meter.phase > 0 ? `L${meter.phase}` : 'N/A'}
          />
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {allChannels.map((ch) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={cn('size-3', isActive ? config.color : '')}
                  />
                  {config.label}
                </button>
              )
            ...
            {meter.protocol === 'mqtt' ? (
              <div className="space-y-2.5">
                ...
                <CopyableField
                  label="Topic"
                  value={
                    config.topic || `meters/${meter.serialNumber}/telemetry`
                  }
                />
...
function InfoRow({
  ...
  return (
    <div>
      ...
      <p className={cn('mt-0.5 text-sm', mono && 'font-mono text-xs')}>
        {value}
      </p>
    </div>
  )
