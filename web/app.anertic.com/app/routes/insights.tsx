...
function generateMockInsights(): InsightsData {
  ...
  const weeklyPattern: HourlyPattern[] = Array.from({ length: 24 }, (_, hour) => {
    const base = 0.8
    ...
    const val = (h: number) =>
      parseFloat(
        Math.max(0.2, base + morningPeak + eveningPeak + solarDip + (Math.random() - 0.5) * h).toFixed(1),
      )
    return {
      hour,
      mon: val(0.8),
    ...
    insights: [
      {
        ...
        summary: "No heartbeat since March 10, 08:14. Estimated daily revenue loss: 45 kWh.",
        detail:
      ...
      },
      {
        ...
        summary: "Moving sessions to 11:00-15:00 could save 8.4 kWh/day in grid imports.",
        detail:
      ...
      },
      {
        ...
        summary: "Clear skies and optimal panel angle pushed production to 24.6 kWh today.",
        detail:
      ...
      },
      {
        ...
        summary: "Charging to 90% by 16:00 from solar surplus eliminates 18:00-21:00 grid dependency.",
        detail:
      ...
      },
      {
        ...
        summary: "Air conditioning load spiked due to thermostat setpoint change. Consider smart scheduling.",
        detail:
      ...
      },
      {
        ...
        summary: "Excess solar exported during midday surplus. Net metering credit applied.",
        detail:
    ...
    anomalies: [
      {
        ...
        deviation: 217,
        severity: "high",
        description: "Grid import surged 3x expected — HVAC compressor cycling detected",
      },
      {
        id: "anom-2",
        ...
        deviation: -44,
        severity: "medium",
        description: "Solar output dropped 44% — cloud cover detected by weather API",
      },
      {
        id: "anom-3",
        ...
        deviation: -57,
        severity: "medium",
        description: "Battery drained faster than predicted — unscheduled EV session",
      },
    ],
    savingsHistory,
    weeklyPattern,
...
function SavingsTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number; dataKey: string; color: string }>; label?: string }) {
    ...
    <div className="rounded-lg border border-border/50 bg-background px-3 py-2 text-xs shadow-xl">
      ...
      {payload.map((entry) => (
        <div key={entry.dataKey} className="flex items-center gap-2">
          ...
          <span className="ml-auto font-medium tabular-nums">{entry.value} kWh</span>
        </div>
      ))}
    </div>
  )
    ...
    >
        ...
        <div className="relative p-6 lg:p-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
            ...
            <div className="flex-1">
              ...
              <p className="mt-3 text-sm leading-relaxed text-foreground/70">{data.dailySummary}</p>
        ...
        <div className="rounded-xl border border-border/50 p-5 lg:col-span-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Actual vs optimal solar self-consumption</p>
              </div>
            ...
            <div className="mt-4 h-52">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.savingsHistory} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gradOptimal" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.72 0.17 163)" stopOpacity={0.15} />
                      <stop offset="100%" stopColor="oklch(0.72 0.17 163)" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="gradActual" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.72 0.17 163)" stopOpacity={0.4} />
                      <stop offset="100%" stopColor="oklch(0.72 0.17 163)" stopOpacity={0.05} />
                    </linearGradient>
                    <linearGradient id="gradGrid" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.7 0.12 250)" stopOpacity={0.3} />
                      <stop offset="100%" stopColor="oklch(0.7 0.12 250)" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.9 0 0)" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} stroke="oklch(0.7 0 0)" />
                  <YAxis tick={{ fontSize: 11 }} stroke="oklch(0.7 0 0)" unit=" kWh" />
        ...
        <div className="rounded-xl border border-border/50 p-5 lg:col-span-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">7-day hourly heatmap (kWh)</p>
              </div>
              ...
              <div className="min-w-[280px]">
                ...
                <div className="mb-1 grid grid-cols-[32px_repeat(7,1fr)] gap-0.5 text-center">
                  ...
                  {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
                    <span key={d} className="text-[9px] font-medium text-muted-foreground/60">
                      {d}
                    </span>
                  ))}
                </div>
                ...
                {data.weeklyPattern
                  ...
                  .map((row) => {
                    ...
                    return (
                      <div key={row.hour} className="grid grid-cols-[32px_repeat(7,1fr)] gap-0.5">
                        <span className="flex items-center text-[9px] tabular-nums text-muted-foreground/50">
                        ...
                        {days.map((val, di) => {
                          ...
                          return (
                            ...
                            >
                              <span className="pointer-events-none absolute -top-7 left-1/2 z-10 hidden -translate-x-1/2 whitespace-nowrap rounded bg-foreground px-1.5 py-0.5 text-[9px] text-background shadow group-hover:block">
                                {val} kWh
                              </span>
                            </div>
                          )
                ...
                <div className="mt-2 flex items-center justify-end gap-1.5">
                  ...
                  {[0.1, 0.25, 0.45, 0.65, 0.85].map((opacity) => (
                    <div
                      ...
                      style={{ backgroundColor: `oklch(0.72 0.17 163 / ${opacity})` }}
                    />
                  ))}
      ...
      <div>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold tracking-tight">Insight Feed</h2>
            <Badge variant="secondary" className="text-[10px]">
              {filteredInsights.length}
            </Badge>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {(["all", "warning", "opportunity", "achievement"] as const).map((filter) => {
          ...
          {filteredInsights.map((insight) => {
            ...
            return (
              ...
              >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex shrink-0 flex-col items-end gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60">
                          <CatIcon className={cn("size-3", categoryColor(insight.category))} />
                ...
                {/* Expanded detail */}
                {isExpanded && (
                  <div className="border-t border-border/50 px-4 pb-4 pt-3 sm:pl-[4.5rem]">
                    <p className="text-xs leading-relaxed text-foreground/70">{insight.detail}</p>
            ...
            {data.anomalies.map((anomaly) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-xs font-medium">{anomaly.description}</p>
                      <Badge className={cn("shrink-0 text-[10px]", sev.bg, sev.text)}>
                        {anomaly.deviation > 0 ? "+" : ""}
                        {anomaly.deviation}%
                      </Badge>
                    </div>
                    <div className="mt-1.5 flex items-center gap-3 text-[10px] text-muted-foreground/60">
                      <span className="tabular-nums">{timeAgo(anomaly.timestamp)}</span>
                      <span>
                        Expected: <strong className="text-foreground/50">{anomaly.expected}</strong>
                      </span>
                      <span>
                        Actual: <strong className={sev.text}>{anomaly.actual}</strong>
                      </span>
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={cn("text-3xl font-bold tabular-nums", textColor)}>{value}</span>
